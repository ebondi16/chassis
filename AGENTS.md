...
# new migration — domain data (ChassisDbContext): run BOTH providers
# (migrations are per-provider assemblies):
...
# new migration — web auth (ChassisAuthDbContext, Identity + OpenIddict, in
# Chassis.Api; SQLite-only for now — see the web-auth isolation rule below):
...
- **Web auth is isolated:** ASP.NET Core Identity + OpenIddict + the
  ...
  same physical database as the domain data) — **SQLite-only today**; a hosted
  Postgres web deployment still needs the auth context taught the provider split
  the domain context already has. The web `ITenantProvider` is
